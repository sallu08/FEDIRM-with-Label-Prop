...
def softmax_mse_weighted_loss1(input_logits, target_logits,sample_weights):
    """Takes softmax on both sides and returns MSE loss
    ...
    input_softmax = F.softmax(input_logits, dim=1)
    target_softmax = F.softmax(target_logits, dim=1)
    print('soft=',target_softmax)
    print('w=',sample_weights)
    target = target_softmax * sample_weights.unsqueeze(-1)
    print('t',target)

    loss = ((input_softmax - target)**2)
  
    return loss

def softmax_mse_weighted_loss2(input_logits, target_logits, sample_weights):
    """Takes softmax on both sides and returns MSE loss
    ...
    # target_softmax = target_softmax * class_weights.expand_as(target_softmax)

    # Apply sample weights
    target_softmax = target_softmax * sample_weights.unsqueeze(-1)

    loss = ((input_softmax - target_softmax)**2)
...
def element_weighted_loss(y_hat, y, weights,class_weights):
    ...
    loss = loss * weights
    return loss
def element_weighted_loss1(y_hat, y):
    assert y_hat.size() == y.size()
...
def softmax_crossentropy_weighted_loss(input_logits, target_logits, sample_weights):
    """Takes softmax on both sides and returns cross-entropy loss
    ...
    cross_entropy_loss = F.cross_entropy(input_softmax, target_softmax, reduction='none')
    weighted_loss = cross_entropy_loss * sample_weights
  
    return weighted_loss


def softmax_crossentropy_weighted_loss1(input_logits, target_logits, sample_weights, class_weights):
    assert input_logits.size() == target_logits.size()
    ...
    cross_entropy_loss = F.cross_entropy(input_softmax, target_softmax, reduction='none')
    weighted_loss = cross_entropy_loss * sample_weights
    weight=weighted_loss*class_weights
